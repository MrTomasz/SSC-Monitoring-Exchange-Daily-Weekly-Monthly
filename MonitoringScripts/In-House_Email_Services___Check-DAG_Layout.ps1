...
$Databases           = [array](Get-MailboxDatabase -IncludePreExchange2013 | ? {$_.Server -like "EIS-*-EP0*" -or $_.Server -like "IMG-*-EP*" -or $_.Server -like "DCS-DCB-EP*"} | Sort Name | Get-MailboxDatabase -Status) 
...
New-Item `
 -ItemType file `
    -Name In-House_Email_Services___DAG_Layout_Report.htm `
      -path $CurrentScriptLocation  `
            -Force

$filename = "$($CurrentScriptLocation)"+ "\In-House_Email_Services___DAG_Layout_Report.htm"
...
<h3 align=""center"">In-House Email Services - DAG Database Copies Layout</h3>
...
$Subject = "In-House Email Services - DAG Layout Report - $(Get-Date -f 'yyyy-MM-dd')"

if (Test-Connection $SMTPServer1 -Quiet)

{Send-MailMessage -from $From -to $Recipient -subject $Subject -SMTPServer $SMTPServer1 -Attachments $fileName}

elseif (Test-Connection $SMTPServer2 -Quiet)

{Send-MailMessage -from $From -to $Recipient -subject $Subject -SMTPServer $SMTPServer2 -Attachments $fileName}

else 
 
{Send-MailMessage -from $From -to $Recipient -subject $Subject -SMTPServer $SMTPServer3 -Attachments $fileName}
